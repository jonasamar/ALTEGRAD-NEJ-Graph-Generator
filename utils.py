...
def preprocess_dataset(dataset, n_max_nodes, spectral_emb_dim, tokenizer, bert_model, device, n_min_nodes=10):
    data_lst = []
    
    if bert_model is not None:
        bert_model.eval()  # Ensure DistilBERT is in evaluation mode

    if dataset == 'test':
        filename = './data/dataset_' + dataset + '.pt'
        desc_file = './data/test/' + dataset + '.txt'
        ...
        if os.path.isfile(filename):
            ...
            for line in fr:
                ...
                # Compute DistilBERT embedding for the prompt
                if tokenizer and bert_model:
                    with torch.no_grad():
                        encoded = tokenizer(desc, return_tensors='pt', padding=True, truncation=True, max_length=512).to(device)
                        bert_output = bert_model(**encoded)
                        bert_embedding = bert_output.last_hidden_state[:, 0, :]  # CLS token embedding
                else:
                    bert_embedding = None

                # Add the prompt embedding to the Data object
                if bert_embedding is not None:
                    data_lst.append(Data(stats=feats_stats, filename=graph_id, prompt_embedding=bert_embedding.cpu()))
                else:
        ...
        if os.path.isfile(filename):
            ...
            for fileread in tqdm(files):
                ...
                # BFS-based adjacency matrix construction
                ...
                # Read and process description for BERT embedding
                if tokenizer and bert_model:
                ...
                # Create the Data object with BERT embeddings
                if bert_embedding is not None:
                    data_lst.append(Data(x=x, edge_index=edge_index, A=adj, stats=feats_stats, filename=filen, prompt_embedding=bert_embedding.cpu()))
                else:
